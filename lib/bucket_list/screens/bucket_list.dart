...
class _BucketListScreenState extends State<BucketListScreen> {
  ...
  Future<Food> fetchFoodDetails(String foodId, CookieRequest request) async {
    try {
      ...
      final response = await request.get('http://10.0.2.2:8000/bucket-list/get-food/$foodId/');
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      final response = await request.post(
                        'http://10.0.2.2:8000/bucket-list/create-bucket-list-flutter/',
                        jsonEncode({
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (editFormKey.currentState!.validate()) {
                      try {
                        final response = await request.post(
                          'http://10.0.2.2:8000/bucket-list/edit-bucket-list-flutter/$bucketListId/',
                          jsonEncode({
                            'name': newName,
                          }),
                        );
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        try {
                          final response = await request.post(
                            'http://10.0.2.2:8000/delete-bucket-list-flutter/$bucketListId/',
